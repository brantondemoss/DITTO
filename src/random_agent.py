...
class Agent:
    def __init__(self, policy_model=None, env=None):
        """
        Args:
            model : policy model
            env : Gym environment
        ...
        if policy_model is None:
            print("No policy provided. Actions will be sampled uniformly at random.")
    ...
    def get_action(self, obs, episode_start=None, hidden_state=None):
        if self.policy_model is None:
            ...
            actions = np.random.uniform(size=(self.env.num_envs, low.shape[0])) * (high - low)  + low
            if actions.shape[0] != 1 or actions.shape[1] != 3:
    ...
    def run_n_episodes(self, n_episodes, savepath=None, max_steps_per_episode=None):
        ...
        episodes = [Episode(last_states[i], info={"img": last_states[0]}) for i in range(n_envs)]
...
if __name__ == "__main__":
    ...
    agent = Agent(env=env)
    agent.run_n_episodes(n_episodes=10, savepath="/home/marc/github/DITTO/episodes/CarRacing-v0")
    print("Hello, World!")